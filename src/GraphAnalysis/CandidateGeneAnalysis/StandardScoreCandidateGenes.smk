...
#             with open(permut_probabilities, "r") as f:
#                 for l in f.readlines()[1:]:
...
rule get_probs_permut_group:
    ...
    input:
        term_mean_var = lambda wc: expand(
            ...
            term = config[wc.group], n = range(config["n_permut"]) )
    output:
        cluster_prob = "work/full-drugbank-benchmark/candidate_genes/group_gene_permut/{group}_permut.csv"
    run:
